...
fileHandle.write ('Time,T1(K),T2(K),T3(K),V1(V),V2(V),Power(W)\n')
fileHandle.close()
...
while True:
    ...
    power = pwr_m.read
    combined_string = str(time_now).strip()+', '+str(round(float(TEMP_922_A.strip()),2))+', '+str(round(float(TEMP_922_B.strip()),2))+', '+str(round(float(TEMP_922_C.strip()),2))+', '+str(DVM_1.strip())+', '+str(DVM_2.strip())+', '+str(power).strip()+'\n'
    fileHandle = open ( FileName, 'a' )
