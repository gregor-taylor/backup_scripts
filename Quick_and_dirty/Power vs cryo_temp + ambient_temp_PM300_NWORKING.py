...
PM300 = rm.open_resource(power_meter_ID)
print(PM300.query("*IDN?"))
PM300.write(":AVER[1], 100")
...
while True:
    ...
    power_1 = PM300.query(":POW[1]:VAL?")
    power_2 = PM300.query(":POW[2]:VAL?")
    combined_string = str(time_now).strip()+', '+str(round(float(TEMP_922_A.strip()),2))+', '+str(round(float(TEMP_922_B.strip()),2))+', '+str(round(float(TEMP_922_C.strip()),2))+', '+str(DVM_1.strip())+', '+str(DVM_2.strip())+', '+str(power_1).strip()+', '+str(power_2).strip()+'\n'
    fileHandle = open ( FileName, 'a' )
