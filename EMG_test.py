...
def fit_exp_mod_gauss(times, counts, dt):
    #convert times to ps
    times=times/1e-12
    ...
    #plt.plot(sliced_times, counts[start_p:end_p],'o', markersize=1)
    fitted_func = exp_mod_gauss(sliced_times, popt[0], popt[1], popt[2], popt[3])
    centre = np.argmax(fitted_func)
    #plt.plot(sliced_times,fitted_func/max(fitted_func),'-')
    #plt.show()
...
#gate arr
d_arr=d_arr[1500:3000]

dt=4.0717e-13
times=np.arange(1, len(d_arr)+1)
times=times*dt          
popt, pcov = fit_exp_mod_gauss(times, d_arr, dt)
#plt.plot(times, d_arr,'o', markersize=1)
#plt.plot(times, exp_mod_gauss(np.arange(4096), popt[0], popt[2], popt[3]), '-')
#print(d_arr)

plt.show()
#plt.plot(exp_mod_gauss())